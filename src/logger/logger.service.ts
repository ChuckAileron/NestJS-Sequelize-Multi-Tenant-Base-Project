...
export class LoggerService {
    ...
    private buildMessage(headers: Record<string, any>): string {
            const messageFinal:string = 'srvRef='+ this.serviceReference + ' txEpd=0'+ ' txRef='+ (headers && headers['x-txref'] || null) + ' cmRef='+ 
                (headers && headers['x-cmref'] || null)+
                ' nodeRef='+ this.nodeReference+ ' rhsRef='+ (headers && headers['x-rhsref'] || null)+ ' chRef='+ (headers && headers['x-chref'] || null)
                + ' prRef='+ (headers && headers['x-prref'] || null)+ ' commerce='+ 
                (headers && headers['x-commerce'] || null)+ ' country='+ (headers && headers['x-country'] || null)+ ' usrTx='+ (headers && headers['x-usrtx'] || null)
